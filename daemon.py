...
class Daemon:
        ...
        def __init__(self, pidfile, stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
            self.stdin  = self.fpath(stdin)
            self.stdout = self.fpath(stdout)
            self.stderr = self.fpath(stderr)
            self.pidfile = self.fpath(pidfile)
            print 'pidfile:', self.pidfile

        def fpath(self, p):
            return p[0] == '/' and p or os.path.abspath(os.curdir)+'/'+p
	...
	def status(self):
	    pid = self.readPid()
	    if pid:
		print 'pid:', pid, self.isAlive(pid)
	    else:
		print 'none pid'
...
if __name__ == "__main__":
    ...
    MyDaemon('./daemon-example.pid', stdout='_1.log').cmd()
